...
try
	{
		...
		$OutputFile = "D:\PDWDiagnostics\TableHealth\LastModifiedStatistics_$CurrTime.txt"
		$OutputFileCSV = "D:\PDWDiagnostics\TableHealth\LastModifiedStatistics_$CurrTime.csv"
		$OutputFileHTML = "D:\PDWDiagnostics\TableHealth\LastModifiedStatistics_$CurrTime.html"
...
try
	{
		...
		$dbs = Invoke-Sqlcmd -Query "select name from sys.databases where name not in ('master','tempdb','stagedb','model','msdb','dwqueue','dwdiagnostics','dwconfiguration') order by name;" -ServerInstance "$PDWHOST" -Username $PDWUID -Password $PDWPWD
	}
catch
	{
