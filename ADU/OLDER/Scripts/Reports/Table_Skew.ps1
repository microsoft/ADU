...
try
	{
		...
		$OutputFile = "c:\PDWDiagnostics\TableHealth\TableSkew_$CurrTime.txt"
		$OutputFileCSV = "c:\PDWDiagnostics\TableHealth\TableSkewReport_$CurrTime.csv"
		$OutputFileHTML = "c:\PDWDiagnostics\TableHealth\TableSkewReport_$CurrTime.html"
		...
		foreach ($db in $databases) 
			{
				...
				try
					{       
						...
						$tbls = Invoke-Sqlcmd -Query "use [$db]; SELECT ta.name TableName FROM sys.tables ta;" -ServerInstance "$PDWHOST,17001" -Username $PDWUID -Password $PDWPWD
					}
				catch
					{
