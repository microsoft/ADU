...
$OutputFile = "c:\PDWDiagnostics\NetworkConfig\NetworkConfig_$CurrTime.txt"
if (!(test-path "c:\PDWDiagnostics\NetworkConfig"))
{
    New-item "c:\PDWDiagnostics\NetworkConfig" -ItemType Dir | Out-Null
}
...
Function GetNetworkAdapterConfig
{
    foreach ($node in $nodelist)
    {
	    ...
	    Write-Host "Retrieving network adapter for $Shortname..."
        "---------------------------------"| out-file -append $OutputFile
		$Shortname | out-file -append $OutputFile
		"---------------------------------"| out-file -append $OutputFile
	    ...
	    if ($node -like "*-AD*")
        {
            Write-host "Getting DHCP server scope from $Shortname..."
			try
			{
