#* FileName: PDWDiag.ps1
#*=============================================
#* Script Name: PDWDiag.ps1
#* Created: [10/28/2013]
...
.EXAMPLE
	.\PDWDiag.ps1 -offline -nodeList
	NodeList mode: Offline mode, the tool will generate a nodeList then present it to you in Notepad to be edited. 
.EXAMPLE
	.\PDWDiag.ps1 -username sa -password P@ssW0rd -actions info_bundle,dm_pdw_waits,health_bundle (Right now the bundles don't work correctly- this is actually an usupported feature)
...
#Because PDWDiag opens in a new window we loose our rootpath variable. Creating it again
...
{
	$fabricDomain = GetFabDomainNameFromXml
	$pdwDomain = GetPdwDomainNameFromXml
	$pdwServer = "$pdwDomain-sqlctl01"
    $port = 17001
}
else
{
	...
	try
	{
	    ...
	    If(!$offline)
	    {
			...
			do
			{
				...
				try
				{
					$goodcreds = CheckPdwCredentials -U $U -P $P -PDWDomain $pdwDomain
				}
				catch
				{
	    ...
	    if(test-path $rootPath\Config\PDWDiag_Input.xml){[xml]$inputFile = Get-Content $rootPath\Config\PDWDiag_Input.xml}
	    else 
	    {
			Write-EventLog -EntryType Error -message "Not able to find input file: $rootPath\Config\PDWDiag_Input.xml" -Source $source -logname ADU -EventID 9999
		    write-error "Not able to find input file: $rootPath\Config\PDWDiag_Input.xml"
	    }
		...
		if (!$FullNodeList)
	    {
			Write-EventLog -EntryType Error -message "GenerateFullNodeListFromXML did not return any nodes!" -Source $source -logname ADU -EventID 9999
		    Write-Error "GenerateFullNodeListFromXML did not return any nodes!"
	    }
	    ...
	    $runId= "PDWDiag_$(get-date -f yyyy-MM-dd_hhmmss)"
		
		#if output directory wasn't provided use the default
		if(!$outputDir)
		{
			$outputDir = "C:\PdwDiagnostics\PDWDiag\$runID"
		}
		else
		{
			$outputDir += "\$runID"
	    ...
	    else
	    {
		    ...
		    foreach ($actionName in $actions)
		    {
			    ...
			    #find the index of the option and action
			    #$UserInput1 = (($inputFile.menu.option.name).indexOf($level1.name) +1)
			    #$userInput2 = (($inputFile.menu.option[$UserInput1-1].action.name).indexOf($level2.name) +1)

			    #if($userInput1 -lt 1 -or $userinput2 -lt 1){Write-Host "Action: $action not found, Skipping"}
			    #else{ExecuteAction $userInput1 $userInput2}
		    }
        ...
        "query_PDW" 
            {
			    ...
			    {		
					...
					try
					{
            	    	ExecutePdwQuery -PdwQuery $($QueryObject.value) -outputpath $outputPath -U $Username -P $Password -pdwDomain $pdwDomain -port $port
					}
					catch
					{
	    ...
	    "query_CTL"
		    {
			    ...
			    if(!$offline)
			    {
				    $node="$pdwdomain-CTL01"
				    mkdir -force "$outputDir\$node\sql_instance\" | Out-Null
				    $outputPath = "$outputDir\$node\sql_instance\$($QueryObject.name).csv"
				    $SqlCtlNode= "$PdwDomain-SQLCTL01"
	    ...
	    "dist_Pshell"
		    {
			    ...
			    {
				    if($node -eq "local"){$currHost = hostname;$FullFQDNNodeList+= $currHost + '.' + $fabricdomain + '.fab.local'}
					$FullNodeList | foreach {
						if($_ -like "*-$node*" -and ($_ -like "*-AD*" -or $_ -like "*-VMM*" -or $_ -like "*-HS*" -or $_ -like "*-ISCSI*")){$FullFQDNNodeList += $_ + '.' + $fabricdomain + '.fab.local'}
						if($_ -like "*-$node*" -and ($_ -like "*-CTL*" -or $_ -like "*-CMP*" -or $_ -like "*-MAD*")){$FullFQDNNodeList += $_ + '.' + $pdwDomain + '.pdw.local'}
					}
			    }
			
			    #make sure there's no duplicates
			    $FullFQDNNodeList = $FullFQDNNodeList | Select -Unique
				...
				try
				{
			    	ExecuteDistributedPowerShell -nodeList $FullFQDNNodeList -command $command -outputPath $outputPath
				}
				catch
				{
