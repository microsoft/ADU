...
function PdwPerfCounters
{
    ...
    do
	{
		...
		$MenuOptions = (
			 ...
			 "Enable Default PDW Performance Counters",
			 "Disable Default PDW Performance Counters",
		...
		Switch($userInput)
		{
			...
			"Enable Default PDW Performance Counters"
			{
                ...
                try
                {
				    EnablePerfCounters -nodeList $CtlNodeList -PdwCtlCounters
				    EnablePerfCounters -nodeList $cmpNodeList -PdwCmpCounters
				    EnablePerfCounters -nodeList $physNodeList -PdwHstCounters
                }
                catch
                {
			...
			"Disable Default PDW Performance Counters"
			{
                ...
                try
                {
				    disablePerfCounters -nodeList $CtlNodeList -PdwCtlCounters
				    disablePerfCounters -nodeList $cmpNodeList -PdwCmpCounters
				    disablePerfCounters -nodeList $physNodeList -PdwHstCounters
                }
                catch
                {
...
Function CreatePerfCounterSet
{
	...
	if ($PdwCtlCounters)
	{
		$command = {logman create counter PdwCtlCounters -ow -f bincirc -max 200 -si 15 -c "\Process(*)\*" "\PhysicalDisk(*)\*" "\Processor(*)\*" "\ProcessorPerformance(*)\*" "\Memory(*)\*" "\System(*)\*" "\Server(*)\*" "\TCPv4(*)\*" "\.NET CLR Memory(sqldwdms)\*" "\Process(sqldwdms)\*" "\Process(sqlservr)\*" "\.NET CLR Memory(sqldweng)\*" "\Process(sqldweng)\*" "\Network Interface(*)\*"}
	...
	if ($PdwHSTCounters)
	{
		$command = {logman create counter PdwHstCounters -ow -f bincirc -max 200 -si 15 -c "\Process(*)\*" "\PhysicalDisk(*)\*" "\Processor(*)\*" "\ProcessorPerformance(*)\*" "\Memory(*)\*" "\System(*)\*" "\Server(*)\*" "\TCPv4(*)\*" "\Network Interface(*)\*"}
		foreach ($node in $nodeList)
		{
			Write-Host -ForegroundColor Cyan "`n$node"
			Invoke-Command -ComputerName $node -ScriptBlock $command
...
{	
	...
	CollectFiles -nodelist $nodeList -filepath "PerfLogs\*" -outputDir "C:\PdwDiagnostics\PDWPerflogs\Perflogs_$date" -days 3
	
	Write-Host -ForegroundColor Cyan "Perf Logs copied to C:\PdwDiagnostics\Perflogs_$date"
}
. PdwPerfCounters
